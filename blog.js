...
(async () => {
  ...
  try {
    ...
    if (post) {
      ...
      list.innerHTML = `<nav class="breadcrumbs" aria-label="Breadcrumb"><a href="index.html">Início</a> <span>›</span> <a href="blog.html">Blog</a> <span>›</span> <span>${esc(post.title)}</span></nav><div class="post-meta">${esc(post.category)} · ${new Date(`${post.date}T12:00:00`).toLocaleDateString('pt-BR')}</div><h1>${esc(post.title)}</h1><p class="article-lead">${esc(post.description)}</p><img src="${esc(post.image)}" alt="${esc(post.title)}" width="720" height="900" loading="eager">${sections}${relatedHtml}${cta}`;
    ...
    list.innerHTML = posts.map((item) => `<article class="post-card"><img src="${esc(item.image)}" alt="${esc(item.title)}" width="720" height="900" loading="lazy"><div class="post-body"><div class="post-meta">${esc(item.category)} · ${new Date(`${item.date}T12:00:00`).toLocaleDateString('pt-BR')}</div><h2>${esc(item.title)}</h2><p>${esc(item.description)}</p><a class="read" aria-label="Ler artigo: ${esc(item.title)}" href="blog.html?post=${encodeURIComponent(item.slug)}">Ler artigo completo →</a></div></article>`).join('');
